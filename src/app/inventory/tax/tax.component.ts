...
export class TaxComponent implements AfterViewInit {
  ...
  columns: ITdDataTableColumn[] = [
    ...
    {name: 'value', label: 'Value', numeric: true, format: v=>v.toFixed(2) },
  ...
  filter(): void {
    ...
    this._taxService.query({__retail_shop_id__in: this.shops, __include: ['retail_shop'],
      __limit: this.pageSize, __page: this.currentPage, __order_by: sortBy})
      .subscribe((resp: {data: Tax[], total: number})=>{
        this.data = resp.data;
